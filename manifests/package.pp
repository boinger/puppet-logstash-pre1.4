  ...
  $java_package      = 'java-1.7.0-openjdk' )
{
  ...
  $logstash_jar = sprintf("%s-%s-%s.%s", "logstash", $logstash_version, $logstash_jar_type, "jar")
  ...
  if $logstash_provider == 'http' {
    ...
    exec { "curl -o $logstash_home/$logstash_jar $logstash_url --cipher rsa_rc4_128_sha":
      timeout => 0,
      cwd     => "/tmp",
